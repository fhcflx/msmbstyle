...
**pinkstyle** provides a set of functions that can be used to demark questions and answers in the the R Markdown document.  They can be used either inline or in code chunks, and the precise usage may vary depending upon the complexity of the text you wish to include.

## Questions

### Simple question

If your question is simply a paragraph of text, you can use `pinkstyle::question()` and provide the paragraph via the argument `text`.  We see an example of this approach below `r tufte::margin_note('The code is shown here, but it would be more typical to hide it via \x60echo = FALSE\x60')`.

```{r, simple-question}
pinkstyle::question(text = "Is this question displaying correctly?")
```

You can also include a question using inline code e.g. `` `r knitr::inline_expr("pinkstyle::question()")` ``.  This is perhaps simpler than using code chunks, since it is unlikely we ever want to display the function call itself.  The question below is generated in this fashion.

`r pinkstyle::question(text = "This is our second question.  It should not be numbered.  Is it?")`
...
`r pinkstyle::question(label = "ques:two", text = "This is our second question.  We expect this to have a number.")`
...
`r pinkstyle::question(text = "Does this question include \x60x\x60 and \x60y\x60?")`
...
`r pinkstyle::question_end()`

## Solutions

In similar fasion to questions, you can insert solutions using either the function `pinkstyle::solution()`, or a combination of `pinkstyle::solution_begin()` & `pinkstyle::solution_end()`.  *Currently solutions can not be labelled or reference in the text.  This is primarily because of an assumption that they would always have an accompanying question that could be referenced, and may change in the future.*

### Simple solution

Again, if your solution is just a single a paragraph of text, you can use `pinkstyle::solution()` and provide the paragraph via the argument `text`.  We see an example of this approach below:

`r pinkstyle::solution(text = "This is a single paragraph providing an answer.")`
...
```{r, simple-solution2}
pinkstyle::solution(text = "This an 'Answer' and cannot be hidden",
                    header = "Answer",
                    toggle = FALSE)
```
